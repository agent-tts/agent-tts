...
type, L-spine ink frame, and centered bold titles with a legend row
above the axes.
...
def _register_pagella():
    ...
    for pattern in (
        os.path.expanduser('~/.TinyTeX/texmf-dist/fonts/opentype/public/tex-gyre/texgyrepagella-*.otf'),
...
def apply_style():
    ...
    plt.rcParams.update({
        ...
        'text.color': INK,
        'axes.grid': False,
        # Titles: centered bold -- the only bold text in a figure.
...
def header_legend(ax, entries, ncol=None, legend_size=9.5,
                  handlelength=2.0, handletextpad=0.3, columnspacing=0.9):
    """Centered per-axes legend row between the bold title and the plot.
    ...
    if title:  # rough reservation; finalize_headers measures the real pad
        ...
        ax.set_title(title, loc='center', fontsize=title_size,
                     pad=8 + rows * (legend_size + 4.5))
    return ax.legend(handles=handles, loc='lower center',
                     bbox_to_anchor=(0.5, 1.0), ncol=n,
...
def fig_header_legend(fig, entries, ncol=None, legend_size=9.5):
    """Centered figure-level legend row above all panels.
...
def finalize_headers(fig, gap=6.0, min_pad=8.0, level_all=True):
    ...
    every centered title to one shared pad (tallest legend + a `gap` on each
    ...
    for ax in fig.axes:
        ...
        if title and (level_all or ax in heights):
            title_size = ax.title.get_fontsize()
            ax.set_title(title, loc='center', fontsize=title_size, pad=pad)
        leg = ax.get_legend()
        if leg is not None:
            ax_h = ax.get_window_extent().height * 72.0 / dpi
            leg.set_bbox_to_anchor((0.5, 1.0 + (pad - gap) / ax_h),
